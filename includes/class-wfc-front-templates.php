...
class WFC_FRONT_TEMPLATES {
	...
	public static function wfc_construct_products_markup( $woocommerce_cart ) {
		...
		foreach ( $woocommerce_cart as $cart_key => $cart_item ) {
			...
			$product_markup  = '<tr>';
			$product_markup .= '<td><a href="' . esc_url( wc_get_cart_remove_url( $cart_key ) ) . '" class="wfc_remove_product sc_remove_icon btn btn-dange" data-product-id="' . filter_var( $product_id, FILTER_SANITIZE_NUMBER_INT ) . '" data-product-sku="' . filter_var( $product_qty, FILTER_SANITIZE_NUMBER_INT ) . '" data-cart_item_key="' . esc_attr( $cart_key ) . '"><i class="fa-solid fa-trash"></i>Remove</a></td>';
